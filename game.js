...
function resetGameStateForNewCareer(teamId, coachName) {
  ...
  // Ao iniciar uma nova carreira precisamos limpar completamente
  // quaisquer dados residuais do jogo anterior. Se não fizermos isso,
  // estruturas como `elenco` e `titulares` podem permanecer na memória
  // global (gameState) e causar telas de tática ou elenco com
  // jogadores do time anterior. Aqui nós criamos o objeto base do
  // gameState com as propriedades essenciais e também zeramos as
  // listas de elenco e titulares. Estas listas serão repovoadas
  // posteriormente pela engine de táticas (Tactics.ensureElencoETitulares)
  // ou pelo carregamento de elenco via Database.
  // reconstruir completamente o objeto de estado para a nova carreira
  gameState = {
    ...
    nextMatchIndex: null,
    // Limpa elenco e titulares para garantir que o novo time
    // carregue corretamente seus jogadores e formação. Essas
    // propriedades serão preenchidas pelo módulo de táticas
    // assim que a tela de escalação for aberta.
    elenco: [],
    titulares: []
  };

  // Também expõe o novo estado no objeto global window.gameState.  Sem
  // isso, alguns módulos (como tactics-ui) continuariam consultando
  // o gameState antigo preso em window.gameState, mantendo elenco e
  // titulares da carreira anterior.  Ao atualizar ambas as
  // referências (variável local e propriedade global) garantimos que
  // o novo time seja carregado corretamente.
  try {
    window.gameState = gameState;
  } catch (e) {
    // em ambientes muito restritos window pode não estar disponível; ignora
  }

  // Atualiza também a propriedade Game.teamId, caso ainda não tenha
  // sido sincronizada pelo main.js. Isso ajuda os módulos a sempre
  // utilizar o teamId correto.
  if (window.Game && typeof window.Game === 'object') {
    window.Game.teamId = teamId;
    window.Game.coachName = coachName || "Técnico";
  }
