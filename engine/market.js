  ...
  const MarketUI = {
    ...
    comprar(jogadorId) {
      ...
      if (window.Market && typeof Market.comprarJogador === "function") {
        ...
        // sincroniza elenco do Game/gameState com novos jogadores
        try {
          if (window.Game && typeof Game.onElencoAtualizado === 'function') {
            Game.onElencoAtualizado();
          }
        } catch (e) {
          console.warn('[MarketUI] Erro ao sincronizar elenco após compra:', e);
        }
        this.atualizarLista();
        return;
      }
      ...
      // Atualiza estruturas de elenco (Game e gameState)
      try {
    ...
    /**
     * Vende um jogador do elenco do usuário.  A venda só é permitida para
     * jogadores cujo `teamId` seja igual ao `Game.teamId`.  O valor
     * recebido é creditado ao saldo do jogador e o atleta passa a
     * ser agente livre (teamId = null).  O elenco em Game e
     * gameState é sincronizado através de Game.onElencoAtualizado().
     * @param {string} jogadorId id do jogador a ser vendido
     */
    venderJogador(jogadorId) {
      if (!jogadorId) return;
      // obtém jogador via banco
      const base = (window.Database && Array.isArray(Database.players)) ? Database.players : (typeof players !== 'undefined' ? players : []);
      const jogador = base.find(p => p.id === jogadorId);
      if (!jogador) {
        alert('Jogador não encontrado.');
        return;
      }
      const meuTimeId = (window.Game && Game.teamId) ? Game.teamId : null;
      if (!meuTimeId || jogador.teamId !== meuTimeId) {
        alert('Não é possível vender um jogador que não pertence ao seu time.');
        return;
      }
      const valor = jogador.value || 0;
      // remove jogador do elenco do Game (caso exista)
      if (Array.isArray(Game.elenco)) {
        Game.elenco = Game.elenco.filter(p => p.id !== jogadorId);
      }
      // atualiza timeId para torná-lo agente livre
      jogador.teamId = null;
      // credita valor na conta do usuário
      if (window.Game) {
        Game.saldo = +(Game.saldo + valor).toFixed(1);
      }
      // sincroniza elenco simplificado em gameState
      try {
        if (typeof Game.onElencoAtualizado === 'function') {
          Game.onElencoAtualizado();
        }
      } catch (e) {
        console.warn('[MarketUI] Erro ao sincronizar elenco após venda:', e);
      }
      alert(`Jogador ${jogador.name || jogador.nome || jogadorId} vendido por ${valor} mi!`);
      // atualiza listas de mercado (jogador vira agente livre) e elenco
      this.atualizarLista();
      if (window.TeamUI && typeof TeamUI.renderSquad === 'function') {
        TeamUI.renderSquad();
      }
      // também atualiza painel de táticas caso esteja aberto
      if (window.TacticsUI && typeof TacticsUI.renderPainel === 'function') {
