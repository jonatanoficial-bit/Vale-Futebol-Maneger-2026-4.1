...
(function () {
  ...
  function prepararProximoJogo() {
    try {
      ...
      let partidaProx = null;
      let contexto = null;
      if (window.Calendar && typeof Calendar.getProximoJogoDoTime === "function") {
        const prox = Calendar.getProximoJogoDoTime(teamId);
        if (prox && prox.match) {
          partidaProx = prox.match;
          contexto = { division: prox.division, roundNumber: prox.roundNumber, matchIndex: prox.matchIndex };
        }
      }
      // Se calendário não existir ou não encontrou partida, procura diretamente nos fixtures
      if (!partidaProx) {
        try {
          ensureFixtures();
          const allTeams = (window.Database?.teams || teams);
          // determina divisão do time atual
          const teamObj = allTeams.find(t => t.id === teamId);
          const division = teamObj?.division || 'A';
          const fs = window.gameState?.fixtures?.[division] || [];
          outerLoop:
          for (let r = 0; r < fs.length; r++) {
            ...
            for (let i = 0; i < rodada.length; i++) {
              ...
              if (!m.played && (m.homeId === teamId || m.awayId === teamId)) {
                partidaProx = m;
                contexto = { division: division, roundNumber: r + 1, matchIndex: i };
                break outerLoop;
              }
            }
          }
        } catch (e) {
          console.warn('[League] Erro ao procurar próximo jogo nos fixtures:', e);
        }
      }
      if (partidaProx) {
        // guarda contexto global para uso ao finalizar partida
        window.currentMatchContext = contexto || {};
        const match = partidaProx;
        const teamsList = (window.Database?.teams || teams);
        const homeTeam = teamsList.find(t => t.id === match.homeId) || { id: match.homeId, name: match.homeId };
        const awayTeam = teamsList.find(t => t.id === match.awayId) || { id: match.awayId, name: match.awayId };
        if (window.Match) {
          Match.state = {
            homeId: match.homeId,
            awayId: match.awayId,
      ...
      // Nenhuma partida encontrada – fallback para iniciar partida aleatória com o time do usuário
      if (window.Match && typeof Match.iniciarProximoJogo === 'function') {
        Match.iniciarProximoJogo();
      }
    } catch (e) {
  ...
  function handleEndOfSeason(finishedDivision) {
    try {
      ...
      // reinicia Copa do Brasil para a nova temporada, se disponível
      try {
        if (window.Cup && typeof Cup.startNewCup === "function") {
          Cup.startNewCup();
        }
      } catch (e) {
        console.warn("[League] Erro ao iniciar nova Copa do Brasil:", e);
      }

      // envia mensagem ao usuário utilizando a modal de campeão se existir
      if (msg) {
        const championYear = ((gameState?.seasonYear) || newYear) - 1;
        const titulo =
          finishedDivision === "A"
            ? `Série A ${championYear}`
            : `Série B ${championYear}`;
        const trophyImg = "assets/geral/trophy_brasileirao.png";
        const mensagemFinal = msg;
        setTimeout(() => {
          if (window.UI && typeof UI.showChampionModal === "function") {
            UI.showChampionModal(titulo, mensagemFinal, trophyImg);
          } else {
            alert(mensagemFinal);
          }
        }, 200);
