...
(function () {
  ...
  function renderSquad() {
    ...
    elenco.forEach((p) => {
      ...
      // monta HTML do cartão com botão de venda
      card.innerHTML = `
        <img src="${imgSrc}" alt="${nome}" onerror="this.style.display='none'">
        <h3>${nome}</h3>
        <p>${pos}</p>
        <p class="ovr">${ovr}</p>
        <button class="btn-principal btn-vender" data-id="${p.id}" style="margin-top:4px;">Vender</button>
      `;
      container.appendChild(card);
    });
    // adiciona listeners aos botões de venda, se houver
    container.querySelectorAll('.btn-vender').forEach(btn => {
      btn.addEventListener('click', (ev) => {
        const id = btn.getAttribute('data-id');
        if (!id) return;
        const confirmar = confirm('Deseja vender este jogador?');
        if (!confirmar) return;
        if (window.MarketUI && typeof MarketUI.venderJogador === 'function') {
          MarketUI.venderJogador(id);
        }
      });
    });
  }
