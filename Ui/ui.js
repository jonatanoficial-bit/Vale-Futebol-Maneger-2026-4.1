...
(function () {
  ...
  // ----------------------------------------------------
  // MODAL DE CAMPEÃO
  // Estas funções permitem exibir uma mensagem de campeão com troféu.
  // A modal é definida no index.html com id="modal-champion".
  // showChampionModal(titulo, mensagem, imgSrc) exibe a modal preenchida.
  // hideChampionModal() esconde a modal.
  // ----------------------------------------------------
  function showChampionModal(titulo, mensagem, imgSrc) {
    const modal = document.getElementById("modal-champion");
    const imgEl = document.getElementById("modal-champion-img");
    const titleEl = document.getElementById("modal-champion-title");
    const msgEl = document.getElementById("modal-champion-message");
    if (!modal || !imgEl || !titleEl || !msgEl) {
      // fallback para alert se a modal não existir
      alert(mensagem);
      return;
    }
    imgEl.src = imgSrc || "";
  ...
  // Registra o clique no botão de fechar quando o documento é carregado
  document.addEventListener("DOMContentLoaded", function () {
    const btnClose = document.getElementById("modal-champion-close");
    if (btnClose) {
      btnClose.addEventListener("click", hideChampionModal);
    }
  });
  ...
  // expõe função de modal no objeto UI para uso pela engine (League/Cup)
